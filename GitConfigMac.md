...
git config --global alias.pl "log --graph --pretty=format:'%C(dim green)%h%Creset -%C(red)%d%Creset %C(cyan)%s%Creset %Cgreen(%cr) %C(dim yellow)<%an>%Creset' --abbrev-commit -10"
